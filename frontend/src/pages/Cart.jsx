...
function Cart() {
  ...
  const handlePayment=async()=>{
    ...
    if(payment?.data?.id){
      stripePromise.redirectToCheckout({sessionId:payment.data.id});
    }
  }
  ...
  return (
    <div className="container mx-auto lg:my-4 p-4">
      <div className="text-center text-lg ">
        {data.length === 0 && !loading && (
          <div className="bg-white p-5 gap-2">
            ...
            <Link
              className="bg-red-500 text-white font-medium p-2 cursor-pointer rounded hover:bg-red-600 transition-all"
              to={"/"}
            >
              Shop Now
            </Link>
      ...
      <div className="flex flex-col lg:flex-row gap-10 lg:justify-between">
        ...
        {/* Total cart cost */}
        {
          data.length>0 && <div className="mb-7 lg:mt-2 w-full lg:max-w-sm">
          {loading ? (
            ...
            <div className="h-36 bg-white ">
              ...
              <Button className="w-full bg-blue-600 p-2 text-white font-medium text-lg hover:bg-blue-700 transition-all" onClick={handlePayment}>Place Order</Button>
            </div>
          )}
        </div>
        }
